...
function openMarkedDocViewer(marked_doc_href) {
	// PFS:
	marked_doc_viewer = window.open("marked_doc_viewer_es.html", '', 'left=20,top=20,width=670,height=500,toolbar=1,resizable=0');
	marked_doc_viewer.onload = function () {
		var ifr = marked_doc_viewer.document.getElementById('iframe_a');
		ifr.src=marked_doc_href;
	};
}
...
politi.model.DebateData = function(data,dl) {
	...
	if(this.loadable) this._url = "../politilines/data/es/" + data.url + "?" + Math.floor(Math.random() * Math.pow(10,10));
	this._loaded = false;
}
...
politi.model.DebateData.prototype.writtenDate = function() {
	...
	return periscopic.locale.Spanish.fullOrdinal(this.date.getDate()) + " de " + periscopic.locale.Spanish.month(this.date.getMonth()) + " de " +  this.date.getFullYear();
}
politi.model.DebateData.prototype.writtenMonthYear = function() {
	...
	return periscopic.locale.Spanish.month(this.date.getMonth()) + ", " + this.date.getFullYear();
}
politi.model.DebateData.prototype.writtenVenue = function() {
	// PFS
	// return this.venue + " in " + this.city;
	return this.venue + " en " + this.city;
}
...
politi.view.WordTooltip = function(uses,wordNode,anchr) {
	...
	link.className = "link";
	// PFS
	link.innerHTML = "explora el uso de esta" + (uses.length > 1?"s":"") + " palabra" + (uses.length > 1?"s":"");
	if(curFilter.pinned == 2 || curFilter.pinned == 0) {
		if(curFilter.pinned == 2) {
			pinCategory = "grupo";
			unpinCategory = "tópico";
		...
		col1.innerHTML = "<div>total:</div><div class='labels'>Este " + pinCategory + ":</div><div class='labels'>Todos los " + pinCategory + "s:</div>";
		var col2 = doc.createElement("div");
		col2.innerHTML = "<div class='labels'>usos</div><div class='gridItem'>" + uses.length + "</div><div class='gridItem'>" + uses[0].word.count + "</div>";
...
politi.view.CandidatePlotTooltip = function(anchr) {
	...
	var showTmr = null;
	// PFS
	content.addClass("plot_tip").html("Este gráfico indica el volumen <br>de las frases de un grupo utilizadas <br>en el transcurso de la sesión. Las <br>apariciones específicas de las palabras <br>se resaltan sobre el 'Top' de palabras.").appendTo(body);
...
politi.view.MenuView.prototype.updateSelectedDebate = function() {
	...
	*/
	var s = StringTools.replace(StringTools.replace(StringTools.replace(StringTools.replace("<span id=\"event_date\">$DATE;</span><br/><strong>$CITY;</strong><br/>$VENUE;","$DATE;",db.detailDate()),"$CITY;",db.city),"$VENUE;",db.venue),"$SPONSORS;",db.detailSponsors());
...
periscopic.locale.Spanish = function() { }
periscopic.locale.Spanish.__name__ = ["periscopic","locale","Spanish"];
periscopic.locale.EnglishUS.month = function(i) {
	// PFS
	return ["January","February","March","April","May","June","July","August","September","October","November","December"][i];
}
periscopic.locale.Spanish.month = function(i) {
	// PFS
...
periscopic.locale.Spanish.fullOrdinal = function(i) {
	return i;
}
...
politi.model.DebateCategories.prototype.load = function() {
	if(!this._inited) {
		...
		var path_debates = "";
		if (el.value == "day") {
			path_debates = "../politilines/data/es/debates_by_day.json";
		}
		else {
			path_debates = "../politilines/data/es/debates_by_month.json";
		}
...
politi.view.WordOverlay = function(uses) {
	...
	if(debaterMode) {
		...
		head.html("<em>" + incept.sentence.block.debater.name + "</em> usó la palabra <em>" + incept.word.word + "</em> <em>" + uses.length + "</em> time" + (uses.length > 1?"s":"") + " relacionada con <em>" + issues.length + "</em> top tópico" + (issues.length > 1?"s":"") + ".");
	} else {
		var debaters = politi.model.DebateData.debatersIn(uses);
		var s = "La palabra <em>" + incept.word.word + "</em> fue usada <em>" + uses.length + "</em> " + (uses.length > 1?"veces":"vez");
		if(curFilter.pinned == 0) s += " por <em>" + debaters.length + "</em> grupo" + (debaters.length > 1?"s":"") + "."; else s += " en esta sesión.";
		head.html(s);
	}
